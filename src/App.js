...
class App extends React.Component {
  ...
  apiCall = () => {
    fetch(
      `http://35.180.182.8/Search?keywords=${this.state.search}&language=${
        navigator.userLanguage || navigator.language.split("-")[0]
      }&limit=${this.state.limitResults}`
    )
      .then((res) => res.json())
      .then((data) =>
        this.setState({
  ...
  render() {
    ...
    return (
      <div onClick={() => this.resultsWindowHandler(false)} className="app">
        ...
        <div className="app-inner">
          ...
          <SearchButton
            searchButtonDisabled={
              !this.state.results || !this.state.results.entries[0]
                ? true
                : false
            }
            textArea={search}
